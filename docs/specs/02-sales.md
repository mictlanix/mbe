...
- **Remove**: hard-deletes the `sales_order_payment` row (`PaymentsController.RemovePayment`). Only available while the order is unpaid and the application unconfirmed; the underlying `customer_payment` is deleted too when it is `Immediate`, or a `CreditPayment` in the current cash session. There is no reversal once the order is paid or the application confirmed.
...
### Purpose
Supervisor tool to correct payment data. Much less capable than its name suggests — it cannot re-apply, split or reassign payments between orders.

### Features
- Search by payment id, sales order id, customer name, or creator/updater first name
- Lists payments with customer, cashier, amount, method, type and linked orders
- Edit a payment's `Amount`, `Method` and `PaymentType`, writing an `incidence` audit row

### Known limitations
- The screen additionally requires `IsAdministrator`, a configured cash drawer and an open cash session, which makes it unreachable from a back-office machine (mictlanix/mbe#48)
- Editing `Amount` does not adjust the `sales_order_payment` applications, so the order balance desynchronises
- The "edit only once" rule is enforced only in the view; the server-side check is commented out
- The audit row records the post-change state rather than the previous one (mictlanix/mbe#42)
- There is no unapply/reverse action
